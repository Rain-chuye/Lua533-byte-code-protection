...
static void LoadFunction (LoadState *S, Proto *f, TString *psource) {
  ...
  if (f->obfuscated) { uint32_t metadata[2] = {(uint32_t)f->linedefined, (uint32_t)f->lastlinedefined}; f->inst_seed = murmurhash3_x86_32(metadata, sizeof(metadata), 0x12345678); }
  LoadOpMap(S, f);
  LoadCode(S, f);
  LoadVCode(S, f);
  LoadConstants(S, f);
