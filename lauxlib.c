...
static const char B85_ALPHABET_MASTER[] =
...
static void shuffle_alphabet85(char *alphabet, unsigned int seed) {
...
LUALIB_API unsigned char *luaL_compress(const unsigned char *input, size_t len, size_t *out_len) {
    ...
    int *head = (int *)malloc(8192 * sizeof(int));
    int *prev = (int *)malloc(65536 * sizeof(int));
    if (!head || !prev) {
        if (head) free(head); if (prev) free(prev);
        free(bs.data); return NULL;
    }
    for (int i = 0; i < 8192; i++) head[i] = -1;

    size_t in_pos = 0;
    while (in_pos < len) {
        int best_len = 0, best_off = 0;
        if (in_pos + 3 <= len) {
            uint32_t h = ((input[in_pos] << 10) ^ (input[in_pos+1] << 5) ^ input[in_pos+2]) & 8191;
            int p = head[h];
            int chain = 0;
            while (p != -1 && in_pos - p <= 32767 && chain < 128) {
                int l = 0;
                while (l < 258 && in_pos + l < len && input[p + l] == input[in_pos + l]) l++;
                if (l > best_len) { best_len = l; best_off = (int)(in_pos - p); if (l == 258) break; }
                p = prev[p & 65535]; chain++;
            }
            prev[in_pos & 65535] = head[h]; head[h] = (int)in_pos;
        }
        if (best_len >= 3) {
            bs_write(&bs, 1, 1);
            if (best_len <= 10) { bs_write(&bs, 0, 1); bs_write(&bs, best_len - 3, 3); }
            else { bs_write(&bs, 1, 1); bs_write(&bs, best_len - 3, 8); }
            bs_write(&bs, best_off - 1, 15);
            for (int k = 1; k < best_len; k++) {
                if (in_pos + k + 3 <= len) {
                    uint32_t h = ((input[in_pos+k] << 10) ^ (input[in_pos+k+1] << 5) ^ input[in_pos+k+2]) & 8191;
                    prev[(in_pos+k) & 65535] = head[h]; head[h] = (int)(in_pos+k);
                }
            }
            in_pos += best_len;
        } else { bs_write(&bs, 0, 1); bs_write(&bs, input[in_pos++], 8); }
    }
    free(head); free(prev);
    bs_flush(&bs);
    *out_len = bs.pos;
...
LUALIB_API unsigned char *luaL_decompress(const unsigned char *input, size_t len, size_t *out_len) {
    if (len == 0) { *out_len = 0; return NULL; }
    BitStream bs;
    bs.pos = 0; bs.bit_buf = 0; bs.bit_cnt = 0;
    size_t orig_len = 0; int shift = 0; unsigned char b;
    do {
        if (bs.pos >= len) return NULL;
        b = input[bs.pos++];
    ...
    while (out_pos < orig_len) {
        if (bs_read(&bs, 1, input, len)) {
            int match_len;
            if (bs_read(&bs, 1, input, len) == 0) match_len = bs_read(&bs, 3, input, len) + 3;
            else match_len = bs_read(&bs, 8, input, len) + 3;
            int off = bs_read(&bs, 15, input, len) + 1;
            if (out_pos < (size_t)off) { free(output); return NULL; }
            for (int i = 0; i < match_len && out_pos < orig_len; i++) {
                output[out_pos] = output[out_pos - off];
                out_pos++;
            }
        } else {
            if (out_pos >= orig_len) break;
...
LUALIB_API unsigned char *luaL_decrypt_chuye(const char *input, size_t in_len, size_t *out_len) {
    ...
    unsigned int seed = 0;
    for (int i = 0; i < 8; i++) {
        int v = b85_value(B85_ALPHABET_MASTER, input[i]);
        if (v == -1) return NULL;
        seed |= ((unsigned int)(v & 0x0F) << (i * 4));
    }
    Xorshift s = {seed, seed ^ 0x92D68CA2, seed ^ 0x475EAD11, seed ^ 0x6E0323B9};
    char alphabet[86]; memcpy(alphabet, B85_ALPHABET_MASTER, 85); alphabet[85] = '\0';
    shuffle_alphabet85(alphabet, seed);
    unsigned char *decoded = (unsigned char *)malloc(in_len + 1);
    if (!decoded) return NULL;
    size_t di = 0; uint32_t bit_buf = 0; int bit_cnt = 0;
    size_t in_pos = 8;
    while (in_pos < in_len) {
        unsigned int r_noise = xorshift128(&s);
        if (r_noise % 13 == 0) { xorshift128(&s); in_pos++; if (in_pos >= in_len) break; }
        if (input[in_pos] == '~') break;
        unsigned int r_data = xorshift128(&s);
        int val_raw = b85_value(alphabet, input[in_pos++]);
        if (val_raw == -1) continue;
        int val = (val_raw + 85 - (int)(r_data % 85)) % 85;
        if (val >= 64) continue;
        bit_buf = (bit_buf << 6) | (val & 0x3F);
        bit_cnt += 6;
        if (bit_cnt >= 8) {
            bit_cnt -= 8;
