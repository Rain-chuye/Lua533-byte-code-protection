...
static int str_char (lua_State *L) {
  ...
  for (i=1; i<=n; i++) {
    ...
    luaL_argcheck(L, uchar(c) == c, i, "值越界");
    p[i - 1] = uchar(c);
  }
    ...
    if (*strfrmt != L_ESC)
      ...
      switch (*strfrmt++) {
        ...
        default: {  /* also treat cases 'pnLlh' */
          return luaL_error(L, "'format' 的选项 '%%%c' 无效",
                               *(strfrmt - 1));
        }
      }
...
static int getnumlimit (Header *h, const char **fmt, int df) {
  ...
  if (sz > MAXINTSIZE || sz <= 0)
    luaL_error(h->L, "整数大小 (%d) 超出限制 [1,%d]",
                     sz, MAXINTSIZE);
  return sz;
}
...
static int str_unpack (lua_State *L) {
  ...
  luaL_argcheck(L, pos <= ld, 3, "初始位置在字符串外");
  initheader(L, &h);
  while (*fmt != '\0') {
    int size, ntoalign;
