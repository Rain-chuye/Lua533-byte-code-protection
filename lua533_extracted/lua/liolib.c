...
static int io_type (lua_State *L) {
  ...
  if (p == NULL)
    ...
    lua_pushliteral(L, "已关闭的文件");
  else
    lua_pushliteral(L, "文件");
  return 1;
}
...
static int f_tostring (lua_State *L) {
  ...
  if (isclosed(p))
    lua_pushliteral(L, "文件 (已关闭)");
  else
    lua_pushfstring(L, "文件 (%p)", p->f);
  return 1;
}
...
static int dir_iter (lua_State *L) {
  ...
  luaL_argcheck (L, d->closed == 0, 1, "已关闭的目录");
#ifdef _WIN32
...
static int dir_iter_factory (lua_State *L) {
  ...
  d->hFile = 0L;
        if (strlen(path) > MAX_PATH-2)
          luaL_error (L, "路径太长: %s", path);
        else
          sprintf (d->pattern, "%s/*", path);
#else
  d->dir = opendir (path);
  if (d->dir == NULL)
    luaL_error (L, "无法打开 %s: %s", path, strerror (errno));
#endif
  return 1;
}
