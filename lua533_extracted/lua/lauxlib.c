...
static void pushfuncname (lua_State *L, lua_Debug *ar) {
  if (pushglobalfuncname(L, ar)) {  /* try first a global name */
    lua_pushfstring(L, "函数 '%s'", lua_tostring(L, -1));
    ...
    lua_pushfstring(L, "函数 <%s:%d>", ar->short_src, ar->linedefined);
  else  /* nothing left... */
    lua_pushliteral(L, "?");
}
...
LUALIB_API void luaL_traceback (lua_State *L, lua_State *L1,
  ...
  lua_pushliteral(L, "栈回溯 (stack traceback):");
...
LUALIB_API int luaL_argerror (lua_State *L, int arg, const char *extramsg) {
  ...
  if (!lua_getstack(L, 0, &ar))  /* no stack frame? */
    return luaL_error(L, "错误参数 #%d (%s)", arg, extramsg);
  lua_getinfo(L, "n", &ar);
  if (strcmp(ar.namewhat, "method") == 0) {
    arg--;  /* do not count 'self' */
    if (arg == 0)  /* error is in the self argument itself? */
      return luaL_error(L, "对不当的 self 调用 '%s' (%s)",
                           ar.name, extramsg);
  }
  if (ar.name == NULL)
    ar.name = (pushglobalfuncname(L, &ar)) ? lua_tostring(L, -1) : "?";
  return luaL_error(L, "对 '%s' 的错误参数 #%d (%s)",
...
static int typeerror (lua_State *L, int arg, const char *tname) {
  ...
  msg = lua_pushfstring(L, "期望 %s，得到 %s", tname, typearg);
...
#define l_inspectstat(stat,what)  \
   ...
   else if (WIFSIGNALED(stat)) { stat = WTERMSIG(stat); what = "信号"; }
...
LUALIB_API int luaL_execresult (lua_State *L, int stat) {
  const char *what = "退出";  /* type of termination */
  if (stat == -1)  /* error? */
...
static int errfile (lua_State *L, const char *what, int fnameindex) {
  ...
  lua_pushfstring(L, "无法 %s %s：%s", what, filename, serr);
...
static int panic (lua_State *L) {
  lua_writestringerror("恐慌 (PANIC): 调用 Lua API 时发生未保护的错误 (%s)\n",
                        lua_tostring(L, -1));
