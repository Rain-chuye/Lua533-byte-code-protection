...
static void seterrorobj (lua_State *L, int errcode, StkId oldtop) {
  switch (errcode) {
    ...
    case LUA_ERRERR: {
      setsvalue2s(L, oldtop, luaS_newliteral(L, "错误处理过程中发生错误"));
      break;
    }
    default: {
...
static void resume (lua_State *L, void *ud) {
  ...
  if (L->status == LUA_OK) {  /* may be starting a coroutine */
    if (ci != &L->base_ci)  /* not in base level? */
      resume_error(L, "不能 resume 非挂起状态的协程", firstArg);
    /* coroutine is in base level; start running it */
    if (!luaD_precall(L, firstArg - 1, LUA_MULTRET))  /* Lua function? */
      luaV_execute(L);  /* call it */
  }
  else if (L->status != LUA_YIELD)
    resume_error(L, "不能 resume 已死亡的协程", firstArg);
...
LUA_API int lua_yieldk (lua_State *L, int nresults, lua_KContext ctx,
  ...
  if (L->nny > 0) {
    if (L != G(L)->mainthread)
      luaG_runerror(L, "尝试跨越 C 调用边界挂起 (yield)");
    else
      luaG_runerror(L, "尝试在协程外部挂起 (yield)");
  }
  L->status = LUA_YIELD;
  ci->extra = savestack(L, ci->func);  /* save current 'func' */
  if (isLua(ci)) {  /* inside a hook? */
    api_check(L, k == NULL, "钩子不能在挂起后继续执行");
  }
  else {
