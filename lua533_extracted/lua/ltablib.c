...
static int tinsert (lua_State *L) {
  ...
  switch (lua_gettop(L)) {
    ...
    case 3: {
      ...
      luaL_argcheck(L, 1 <= pos && pos <= e, 2, "位置越界");
    ...
    default: {
      return luaL_error(L, "对 'insert' 的参数个数错误");
    }
  }
...
static int tremove (lua_State *L) {
  ...
  if (pos != size)  /* validate 'pos' if given */
    luaL_argcheck(L, 1 <= pos && pos <= size + 1, 1, "位置越界");
...
static void addfield (lua_State *L, luaL_Buffer *b, lua_Integer i) {
  lua_geti(L, 1, i);
  if (!lua_isstring(L, -1))
    luaL_error(L, "'concat' 的表中索引 %d 处的值无效 (%s)",
                  i, luaL_typename(L, -1));
  luaL_addvalue(b);
}
...
static int unpack (lua_State *L) {
  ...
  if (n >= (unsigned int)INT_MAX  || !lua_checkstack(L, (int)(++n)))
    return luaL_error(L, "解包 (unpack) 的结果太多");
...
static int sort (lua_State *L) {
  ...
  if (n > 1) {  /* non-trivial interval? */
    luaL_argcheck(L, n < INT_MAX, 1, "数组太大");
