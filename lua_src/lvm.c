...
/* Hidden decryption logic */
static inline Instruction LUAV_DECODE(Instruction v, int idx, Proto *p) {
  if (!p->obfuscated) return v;
  uint32_t _v = (uint32_t)v;
  uint32_t _k = p->inst_seed ^ PROTO_TS(p) ^ PROTO_SALT(p) ^ (uint32_t)(p->sizecode ^ p->sizek ^ (p->numparams << 16));
  _k = (_k ^ (_k >> 16)) * 0x85ebca6b; _k = (_k ^ (_k >> 13)) * 0xc2b2ae35; _k ^= (_k >> 16);
  uint32_t _dk = _k ^ ((uint32_t)idx * 0x9E3779B9U);
  _v ^= 0x55555555;
  _v = (_v >> 7) | (_v << 25);
  _v += 0x12345678;
  _v ^= _dk;
...
static void decrypt_tv(TValue *o, const Proto *f) {
  if (!f->obfuscated) return;
  if (ttisinteger(o)) {
    uint64_t _k = ((uint64_t)PROTO_TS(f) << 32) | PROTO_SALT(f);
    _k ^= 0xDEADBEEFCAFEBABEULL;
    uint64_t _v = (uint64_t)o->value_.i ^ _k;
    _v = (_v - 0x123456789ABCDEF0ULL) * 0xaaaaaaaaaaaaaaabULL;
    o->value_.i = (lua_Integer)_v;
  } else if (ttisfloat(o)) {
    union { lua_Number f; uint64_t i; } _u;
    uint64_t _k = (((uint64_t)PROTO_TS(f) << 32) | PROTO_SALT(f)) ^ 0xFEEDFACEDEADBEEFULL;
    _u.f = o->value_.n;
    _u.i ^= _k;
    o->value_.n = (_u.f - 12345.6789) / 3.14159265;
  }
}

static TValue *get_rk_ptr(TValue *k, int arg, TValue *tmp, const Proto *f) {
  const TValue *c = &k[INDEXK(arg)];
  if (f->obfuscated && ttisnumber(c)) {
    *tmp = *c;
    decrypt_tv(tmp, f);
    return tmp;
  }
...
#define RKB(i)	check_exp(getBMode(GET_REAL_OPCODE(i, cl->p)) == OpArgK, \
	ISK(GETARG_B(i)) ? (cl->p->obfuscated ? get_rk_ptr(k, GETARG_B(i), base + cl->p->scratch_base, cl->p) : k+INDEXK(GETARG_B(i))) : base+GETARG_B(i))
#define RKC(i)	check_exp(getCMode(GET_REAL_OPCODE(i, cl->p)) == OpArgK, \
	ISK(GETARG_C(i)) ? (cl->p->obfuscated ? get_rk_ptr(k, GETARG_C(i), base + cl->p->scratch_base + 1, cl->p) : k+INDEXK(GETARG_C(i))) : base+GETARG_C(i))
...
#define donextjump(ci)	{ \
    i = *ci->u.l.savedpc; \
    i = LUAV_DECODE(i, (int)(ci->u.l.savedpc - cl->p->code), cl->p); \
    dojump(ci, i, 1); \
}
...
#define vmfetch()	{ \
  ...
  i = *(ci->u.l.savedpc++); \
  i = LUAV_DECODE(i, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p); \
...
void luaV_execute (lua_State *L) {
  ...
  static const void *const jump_table[NUM_OPCODES] = {
    ...
    &&L_OP_FUSE_ADD_TO_FIELD, &&L_OP_FUSE_MULADD
  };
#endif
  CallInfo *ci = L->ci;
  LClosure *cl;
  ...
  for (;;) {
    L_next_ins: {
      ...
      if (__builtin_expect(vpc && vcount > 0, 0)) {
        Instruction _vi = LUAV_DECODE(*vpc, vpc_idx, cl->p) ^ 0xAAAAAAAA;
        vpc++; vpc_idx++; vcount--;
        uint32_t _ta = (_vi >> 15) & 0xFF;
        uint32_t _to = (_vi >> 9) & 0x3F;
        uint32_t _tc = _vi & 0x1FF;
        uint32_t _tb = (_vi >> 23) & 0x1FF;
        i = _to | (_ta << 6) | (_tc << 14) | (_tb << 23);
        ra = base + _ta;
      } else {
        vpc = NULL;
        i = *ci->u.l.savedpc++;
      ...
      vmcase(OP_LOADKX) {
        ...
        next_i = LUAV_DECODE(next_i, (int)(ci->u.l.savedpc - cl->p->code), cl->p);
      ...
      vmcase(OP_FUSE_MULADD) {
        Instruction next_i = *ci->u.l.savedpc++;
        next_i = LUAV_DECODE(next_i, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p);
        int rd_idx = GETARG_Ax(next_i);
        TValue *rb = RB(i);
        TValue *rc = RKC(i);
        TValue *rd = ISK(rd_idx) ? get_rk_ptr(k, RKASK(rd_idx), ra + 1, cl->p) : base + rd_idx;
        if (ttisinteger(rb) && ttisinteger(rc) && ttisinteger(rd)) {
            setivalue(ra, ivalue(rb) * ivalue(rc) + ivalue(rd));
        } else {
            lua_Number nb = 0, nc = 0, nd = 0;
            tonumber(rb, &nb); tonumber(rc, &nc); tonumber(rd, &nd);
            setfltvalue(ra, luai_numadd(L, luai_nummul(L, nb, nc), nd));
        }
        vmbreak;
      }
      vmcase(OP_NEWARRAY) {
      ...
      vmcase(OP_FUSE_PARTICLE_DIST) {
        ...
        next_i1 = LUAV_DECODE(next_i1, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p);
        Instruction next_i2 = *ci->u.l.savedpc++;
        next_i2 = LUAV_DECODE(next_i2, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p);
      ...
      vmcase(OP_FUSE_ADD_TO_FIELD) {
        ...
        TValue *rk_key = get_rk_ptr(k, GETARG_B(i), base + cl->p->scratch_base, cl->p);
        TValue *rk_val = get_rk_ptr(k, GETARG_C(i), base + cl->p->scratch_base + 1, cl->p);
      ...
      vmcase(OP_TERNARY) {
        ...
        if (!l_isfalse(rb)) {
          ...
          rc_val = ISK(b) ? get_rk_ptr(k, b, base + cl->p->scratch_base + 1, cl->p) : base + b;
        }
      ...
      vmcase(OP_VIRTUAL) {
        ...
        if (cl->p->vcode && vindex >= 0 && vindex < cl->p->sizevcode) {
          ...
          vcount = (int)LUAV_DECODE(vcount_raw, vpc_idx++, cl->p);
      ...
      vmcase(OP_FUSE_GETSUB) {
        ...
        TValue *rd_val = ISK(rd_idx) ? get_rk_ptr(k, RKASK(rd_idx), v_res + 1, cl->p) : base + rd_idx;
      ...
      vmcase(OP_FUSE_GETGETSUB) {
        ...
        tmp2 = tmp1 + 1;
        gettableProtected(L, base + re, ISK(rf) ? get_rk_ptr(k, rf, tmp2 + 1, cl->p) : base + rf, tmp2);
        ra = RA(i);
        tmp1 = base + cl->p->scratch_base;
        tmp2 = tmp1 + 1;
