...
static void fuse_instructions_internal(Proto *f) {
    ...
    /* Pattern: MUL + ADD -> FUSE_MULADD */
    for (int i = 0; i < f->sizecode - 1; i++) {
        Instruction inst1 = f->code[i];
        Instruction inst2 = f->code[i+1];
        if (GET_OPCODE(inst1) == OP_MUL && GET_OPCODE(inst2) == OP_ADD) {
            int ra1 = GETARG_A(inst1);
            int ra2 = GETARG_A(inst2);
            int rb2 = GETARG_B(inst2);
            int rc2 = GETARG_C(inst2);
            if (ra1 == rb2) {
                f->code[i] = CREATE_ABC(OP_FUSE_MULADD, ra2, GETARG_B(inst1), GETARG_C(inst1));
                f->code[i+1] = CREATE_Ax(OP_EXTRAARG, rc2);
                i++;
            }
            else if (ra1 == rc2) {
                f->code[i] = CREATE_ABC(OP_FUSE_MULADD, ra2, GETARG_B(inst1), GETARG_C(inst1));
                f->code[i+1] = CREATE_Ax(OP_EXTRAARG, rb2);
                i++;
            }
        }
    }
...
void obfuscate_proto(lua_State *L, Proto *f, int encrypt_k) {
    ...
    SET_PROTO_TS(f, (unsigned int)time(NULL));
    SET_PROTO_SALT(f, (unsigned int)rand());
    if (!seeded) {
    ...
    if (encrypt_k) {
        for (int i = 0; i < f->sizek; i++) {
            TValue *o = &f->k[i];
            if (ttisinteger(o)) {
                o->value_.i = ENCRYPT_INT(o->value_.i, f);
            } else if (ttisfloat(o)) {
                o->value_.n = ENCRYPT_FLT_VAL(o->value_.n, f);
            }
        }
    }
    ...
    while (vptr < f->sizevcode) {
        ...
        for (int j = 0; j < count; j++) {
            ...
            /* Obscure Layout: C[0..8] | OP[9..14] | A[15..22] | B[23..31] */
            Instruction vinst = (c) | (op << 9) | (a << 15) | (b << 23);
            f->vcode[vptr++] = vinst ^ 0xAAAAAAAA;
        }
    }
    ...
    for (int i = 0; i < f->sizecode; i++) {
        f->code[i] = ENCRYPT_INST(f->code[i], i, f);
    }
    vptr = 0;
    while (vptr < f->sizevcode) {
        int vcount_idx = vptr;
        ...
        f->vcode[vcount_idx] = ENCRYPT_INST(f->vcode[vcount_idx], vcount_idx, f);
        for (int j = 0; j < count; j++) {
            f->vcode[vptr] = ENCRYPT_INST(f->vcode[vptr], vptr, f);
            vptr++;
        }
    }
