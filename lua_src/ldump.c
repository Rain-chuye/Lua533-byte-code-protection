...
static void DumpString (const TString *s, DumpState *D) {
  if (s == NULL)
    ...
    size_t i;
    uint32_t _k = PROTO_TS(D->f) ^ PROTO_SALT(D->f) ^ 0xAA55AA55;
    lu_byte key = (lu_byte)((_k ^ (_k >> 8) ^ (_k >> 16) ^ (_k >> 24)) & 0xFF);
    for (i = 0; i < size - 1; i++) {
        DumpByte((lu_byte)((lu_byte)str[i] ^ key), D);
    }
  }
}
...
static void DumpHeader (DumpState *D) {
  DumpLiteral(LUA_CUSTOM_SIGNATURE, D);
