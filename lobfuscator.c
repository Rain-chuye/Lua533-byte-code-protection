...
void obfuscate_proto(lua_State *L, Proto *f, int encrypt_k) {
    ...
    uint32_t timestamp = (uint32_t)time(NULL); uint32_t salt = (uint32_t)rand(); uint32_t metadata[2] = {timestamp, salt}; f->inst_seed = murmurhash3_x86_32(metadata, sizeof(metadata), 0x12345678); f->linedefined = (int)timestamp; f->lastlinedefined = (int)salt;
