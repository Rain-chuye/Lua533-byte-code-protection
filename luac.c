...
static void PrintCode(const Proto* f)
{
 ...
 for (pc=0; pc<n; pc++)
 {
  ...
  if (f->obfuscated) { uint32_t _k = (f->inst_seed) ^ ((uint32_t)(pc) * 0x9E3779B9U); if (((_k | ~_k) & 0xFFFFFFFF) == 0xFFFFFFFF) { i -= _k; i = (i >> 13) | (i << 19); i ^= _k; } }
  ...
  switch (o)
  {
   ...
   case OP_SETLIST:
    if (c==0) {
        ...
        if (f->obfuscated) { uint32_t _k = (f->inst_seed) ^ ((uint32_t)(pc) * 0x9E3779B9U); if (((_k | ~_k) & 0xFFFFFFFF) == 0xFFFFFFFF) { next_i -= _k; next_i = (next_i >> 13) | (next_i << 19); next_i ^= _k; } }
        printf("\t; %d",(int)next_i);
    } else printf("\t; %d",c);
    break;
   case OP_TERNARY:
    ...
    printf(" ");
    {
      ...
      if (f->obfuscated) { uint32_t _k = (f->inst_seed) ^ ((uint32_t)(pc + 1) * 0x9E3779B9U); if (((_k | ~_k) & 0xFFFFFFFF) == 0xFFFFFFFF) { next_i -= _k; next_i = (next_i >> 13) | (next_i << 19); next_i ^= _k; } }
