...
#define RA(i)	(ci->u.l.base+GETARG_A(i))
...
void luaV_execute (lua_State *L) {
  ...
  static const void *const jump_table[NUM_OPCODES] = {
    ...
    &&L_OP_TERNARY, &&L_OP_TESTNIL, &&L_OP_SPACESHIP, &&L_OP_VIRTUAL, &&L_OP_FUSE_GETSUB, &&L_OP_FUSE_GETADD, &&L_OP_FUSE_GETGETSUB, &&L_OP_FAST_DIST, &&L_OP_FUSE_NOP, &&L_OP_FUSE_PARTICLE_DIST,
  ...
  for (;;) {
    L_next_ins: {
      ...
      if (__builtin_expect(vpc && vcount > 0, 0)) {
        ...
        /* Decode Randomized Custom ISA: randomized field order based on function seed */
        int order = cl->p->inst_seed % 6;
        Instruction vi = ~raw_i;
        OpCode v_op = (OpCode)(vi & 0x3F);
        int v_a, v_b, v_c;
        switch(order) {
          case 0: v_a = (vi >> 6) & 0xFF; v_b = (vi >> 14) & 0x1FF; v_c = (vi >> 23) & 0x1FF; break;
          case 1: v_a = (vi >> 6) & 0xFF; v_c = (vi >> 14) & 0x1FF; v_b = (vi >> 23) & 0x1FF; break;
          case 2: v_b = (vi >> 6) & 0x1FF; v_a = (vi >> 15) & 0xFF; v_c = (vi >> 23) & 0x1FF; break;
          case 3: v_b = (vi >> 6) & 0x1FF; v_c = (vi >> 15) & 0x1FF; v_a = (vi >> 24) & 0xFF; break;
          case 4: v_c = (vi >> 6) & 0x1FF; v_a = (vi >> 15) & 0xFF; v_b = (vi >> 23) & 0x1FF; break;
          case 5: v_c = (vi >> 6) & 0x1FF; v_b = (vi >> 15) & 0x1FF; v_a = (vi >> 24) & 0xFF; break;
          default: v_a=v_b=v_c=0; break;
        }
      ...
      vmcase(OP_FUSE_PARTICLE_DIST) {
        ...
        gettableProtected(L, p1, kx, base + cl->p->scratch_base + 2);
        gettableProtected(L, base + p2_idx, kx, base + cl->p->scratch_base + 2 + 1);
        gettableProtected(L, base + p1_idx, ky, base + cl->p->scratch_base + 2 + 2);
        gettableProtected(L, base + p2_idx, ky, base + cl->p->scratch_base + 2 + 3);
        ra = RA(i);
        StkId tmp = base + cl->p->scratch_base + 2;
        lua_Number n1=0, n2=0, n3=0, n4=0;
      ...
      vmcase(OP_LE) {
        ...
        TValue *rb = RKB(i);
        Protect(
          if (luaV_lessequal(L, rb, rc) != GETARG_A(i))
            ci->u.l.savedpc++;
          else
            donextjump(ci);
        )
      ...
      vmcase(OP_FUSE_GETGETSUB) {
        ...
        int rf = extra & 0x1FF;
        StkId tmp1 = base + cl->p->scratch_base + 2;
        StkId tmp2 = tmp1 + 1;
        /* First table access */
