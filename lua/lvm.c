...
static OpCode chuye_get_op(Instruction i, Proto *p) {
  uint32_t op = (uint32_t)GET_OPCODE(i);
  if (p->op_map) op = p->op_map[op & 0x3F];
  return (OpCode)(op & 0x3F);
}
#define GET_REAL_OPCODE(i, p) chuye_get_op(i, p)
...
/* Obfuscated Decryption Helper */
static Instruction chuye_vm_decode(Instruction i, int idx, uint32_t seed) {
  uint32_t k = seed ^ ((uint32_t)idx * 0x9E3779B9U);
  uint32_t v = i - k;
  uint32_t noise = (k ^ v) & 0;
  v = (v >> 13) | (v << 19);
  if ((k | ~k) == 0xFFFFFFFF) {
      uint32_t r = chuye_rsbox[v & 0xFF];
      r |= (uint32_t)chuye_rsbox[(v >> 8) & 0xFF] << 8;
      r |= (uint32_t)chuye_rsbox[(v >> 16) & 0xFF] << 16;
      r |= (uint32_t)chuye_rsbox[(v >> 24) & 0xFF] << 24;
      v = r;
  }
  return (v ^ k) + noise;
}
...
#define donextjump(ci)	{ \
    i = *ci->u.l.savedpc; \
    i = chuye_vm_decode(i, (int)(ci->u.l.savedpc - cl->p->code), cl->p->inst_seed); \
    dojump(ci, i, 1); \
}
...
#define vmfetch()	{ \
  ...
  i = *(ci->u.l.savedpc++); \
  i = chuye_vm_decode(i, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p->inst_seed); \
...
void luaV_execute (lua_State *L) {
  ...
  static const void *const jump_table[NUM_OPCODES] = {
    ...
    && L_OP_FUSE_ADD_TO_FIELD, && L_OP_FUSE_GETTABLE_TEST, && L_OP_FUSE_GETTABLE_EQ
  };
#endif
  CallInfo *ci = L->ci;
  LClosure *cl;
  ...
  for (;;) {
    L_next_ins: {
      ...
      if (__builtin_expect(ci->u.l.vpc && ci->u.l.vcount > 0, 0)) {
        ...
        raw_i = chuye_vm_decode(raw_i, ci->u.l.vpc_idx++, cl->p->inst_seed);
      ...
      vmcase(OP_LOADKX) {
        ...
        next_i = chuye_vm_decode(next_i, (int)(ci->u.l.savedpc - cl->p->code), cl->p->inst_seed);
      ...
      vmcase(OP_FUSE_PARTICLE_DIST) {
        ...
        next_i1 = chuye_vm_decode(next_i1, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p->inst_seed);
        Instruction next_i2 = *ci->u.l.savedpc++;
        next_i2 = chuye_vm_decode(next_i2, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p->inst_seed);
        ...
        if (ttistable(p1) && ttistable(p2) && ttisshrstring(kx) && ttisshrstring(ky)) {
            ...
            if (s1 && !ttisnil(s1) && s2 && !ttisnil(s2) && s3 && !ttisnil(s3) && s4 && !ttisnil(s4)) {
        ...
        ra = RA(i);
        {
          ...
          lua_Number _n1=0, _n2=0, _n3=0, _n4=0;
          if (tonumber(tmp, &_n1) && tonumber(tmp+1, &_n2) && tonumber(tmp+2, &_n3) && tonumber(tmp+3, &_n4)) {
              setfltvalue(ra, sqrt((_n1-_n2)*(_n1-_n2) + (_n3-_n4)*(_n3-_n4)));
      ...
      vmcase(OP_FUSE_ADD_TO_FIELD) {
        ...
        TValue *rk_key = ISK(GETARG_B(i)) ? k + INDEXK(GETARG_B(i)) : base + GETARG_B(i);
        TValue *rk_val = ISK(GETARG_C(i)) ? k + INDEXK(GETARG_C(i)) : base + GETARG_C(i);
      ...
      vmcase(OP_TERNARY) {
        TValue *rb = RB(i);
        Instruction next_i = *ci->u.l.savedpc++;
        next_i = chuye_vm_decode(next_i, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p->inst_seed);
        const TValue *rc_val;
        if (!l_isfalse(rb)) {
          rc_val = RKC(i);
        } else {
          int b = GETARG_Bx(next_i);
          rc_val = ISK(b) ? k + INDEXK(b) : base + b;
        }
      ...
      vmcase(OP_VIRTUAL) {
        ...
        if (cl->p->vcode && vindex >= 0 && vindex < cl->p->sizevcode) {
          ...
          ci->u.l.vcount = (int)chuye_vm_decode(vcount_raw, ci->u.l.vpc_idx++, cl->p->inst_seed);
          ci->u.l.savedpc += (ci->u.l.vcount - 1);
        }
        vmbreak;
      }
      vmcase(OP_FUSE_GETSUB) {
        ...
        TValue *rd_val = ISK(rd_idx) ? k + INDEXK(RKASK(rd_idx)) : base + rd_idx;
      ...
      vmcase(OP_FUSE_GETTABLE_TEST) {
        ...
        Instruction next_i = *ci->u.l.savedpc++;
        next_i = chuye_vm_decode(next_i, (int)(ci->u.l.savedpc - cl->p->code - 1), cl->p->inst_seed);
        int test_c = GETARG_Ax(next_i);
        gettableProtected(L, rb, rc, ra);
        if (test_c ? l_isfalse(ra) : !l_isfalse(ra))
            ci->u.l.savedpc++;
        else {
            i = *ci->u.l.savedpc;
            i = chuye_vm_decode(i, (int)(ci->u.l.savedpc - cl->p->code), cl->p->inst_seed);
      ...
      vmcase(OP_FUSE_GETTABLE_EQ) {
        ...
        int extra = GETARG_Ax(next_i);
        int eq_a = (extra >> 9) & 1;
        int rd_idx = extra & 0x1FF;
        gettableProtected(L, rb, rc, ra);
        TValue *rd = ISK(rd_idx) ? k + INDEXK(rd_idx) : base + rd_idx;
        if (luaV_equalobj(L, ra, rd) != eq_a)
            ci->u.l.savedpc++;
        else {
            i = *ci->u.l.savedpc;
            i = chuye_vm_decode(i, (int)(ci->u.l.savedpc - cl->p->code), cl->p->inst_seed);
