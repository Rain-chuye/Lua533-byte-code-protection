...
void luaV_finishget (lua_State *L, const TValue *t, TValue *key, StkId val,
  ...
  luaG_runerror(L, "'__index' chain too long; possible loop (PS: '__index' 链太长；可能存在循环)");
}
...
void luaV_finishset (lua_State *L, const TValue *t, TValue *key,
  ...
  luaG_runerror(L, "'__newindex' chain too long; possible loop (PS: '__newindex' 链太长；可能存在循环)");
}
  ...
  switch (ttype(rb)) {
    ...
    default: {  /* try metamethod */
      ...
      if (ttisnil(tm))  /* no metamethod? */
        luaG_typeerror(L, rb, "get length of (PS: 获取长度)");
      break;
    }
  }
...
lua_Integer luaV_div (lua_State *L, lua_Integer m, lua_Integer n) {
  if (l_castS2U(n) + 1u <= 1u) {  /* special cases: -1 or 0 */
    if (n == 0)
      luaG_runerror(L, "attempt to divide by zero (PS: 尝试除以零)");
...
lua_Integer luaV_mod (lua_State *L, lua_Integer m, lua_Integer n) {
  if (l_castS2U(n) + 1u <= 1u) {  /* special cases: -1 or 0 */
    if (n == 0)
      luaG_runerror(L, "attempt to perform 'n%%0' (PS: 尝试对 0 取模)");
...
#if (defined(__GNUC__) || defined(__clang__)) && !defined(LUA_USE_JUMP_TABLE)
#define LUA_USE_JUMP_TABLE 1
#endif

#if defined(LUA_USE_JUMP_TABLE)
#define vmdispatch(o)	goto *dispatch_table[o]; if (0)
...
void luaV_execute (lua_State *L) {
  ...
  static const void *const dispatch_table[] = {
    ...
    &&L_OP_NEWARRAY, &&L_OP_TFOREACH, &&L_OP_GETTABLE_CHAIN, &&L_OP_VIRTUAL
  };
#endif
  CallInfo *ci = L->ci;
  LClosure *cl;
  ...
  L_next_ins:
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_ADD) {
        ...
        if (l_likely(ttisinteger(rb) && ttisinteger(rc))) {
      ...
      vmcase(OP_GETTABLE_CHAIN) {
        int n = GETARG_C(i);
        StkId rb = base + GETARG_B(i);
        setobjs2s(L, ra, rb);
        for (int j = 0; j < n; j++) {
            Instruction extra = *(ci->u.l.savedpc++);
            TValue *key = k + GETARG_Ax(extra);
            const TValue *slot;
            if (luaV_fastget(L, ra, key, slot, luaH_get)) {
                setobj2s(L, ra, slot);
            } else {
                Protect(luaV_finishget(L, ra, key, ra, slot));
            }
        }
        vmbreak;
      }
