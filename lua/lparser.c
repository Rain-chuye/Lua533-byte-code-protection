...
static void fieldchain (LexState *ls, expdesc *v) {
  FuncState *fs = ls->fs;
  int keys[100];
  int n = 0;
  ...
  while (ls->t.token == '.') {
    ...
    keys[n++] = luaK_stringK(fs, name);
    if (n >= 100 || ls->t.token != '.') break;
  }
  if (n > 1) {
    init_exp(v, VRELOCABLE, luaK_codeABC(fs, OP_GETTABLE_CHAIN, 0, base, n));
    for (int i = 0; i < n; i++)
      luaK_code(fs, CREATE_Ax(OP_EXTRAARG, keys[i]));
  } else {
    expdesc key;
    init_exp(&key, VK, keys[0]);
    luaK_indexed(fs, v, &key);
  }
}
