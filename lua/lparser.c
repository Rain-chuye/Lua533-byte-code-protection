...
static void close_func (LexState *ls) {
  ...
  f->maxstacksize += 10; // Extra space for RK decryption and fusion
...
static void lambda_body(LexState *ls, expdesc *e, int line) {
    ...
    open_func(ls, &new_fs, &bl);
    lambda_parlist(ls);
    if (testnext(ls, TK_LET)||testnext(ls, ':') || testnext(ls, TK_MEAN)) {
        if (ls->t.token == '{') {
            block(ls);
        } else {
            enterlevel(ls);
            expdesc ev;
            expr(ls, &ev);
            luaK_ret(&new_fs, luaK_exp2anyreg(&new_fs, &ev), 1);
  ...
  switch (ls->t.token) {
    ...
    case TK_NEW: {
      ...
      while (ls->t.token == '.' || ls->t.token == '[') {
          if (ls->t.token == '.') fieldsel(ls, v);
          else yindex(ls, v);
      }
      expdesc f;
      TString *newinst = luaS_new(ls->L, "newinstance");
      singlevaraux(fs, newinst, &f, 1);
      if (f.k == VVOID) {
          expdesc key;
          singlevaraux(fs, ls->envn, &f, 1);
          codestring(ls, &key, newinst);
          luaK_indexed(fs, &f, &key);
      }
      luaK_exp2nextreg(fs, &f);
      int base = f.u.info;
      luaK_exp2nextreg(fs, v);
      if (ls->t.token == '(' || ls->t.token == '{' || ls->t.token == TK_STRING)
        funcargs(ls, &f, line);
      else {
        init_exp(&f, VCALL, luaK_codeABC(fs, OP_CALL, base, 2, 2));
      }
      *v = f;
      return;
    }
    case TK_SUPER: {
      luaX_next(ls);
      TString *super_s = luaS_new(ls->L, "super");
      singlevaraux(ls->fs, super_s, v, 1);
      if (v->k == VVOID) {
          // Fallback to global if not found as local/upvalue (should not happen in proper class methods)
          singlevaraux(ls->fs, ls->envn, v, 1);
          expdesc key;
          codestring(ls, &key, super_s);
  ...
  for (;;) {
    switch (ls->t.token) {
      ...
      case TK_PPLUS: {
        ...
        expdesc v_old;
        expdesc e_one;
        check_condition(ls, vkisvar(v->k), "期望变量");
        luaX_next(ls);
        v_old.k = VNONRELOC;
        v_old.u.info = fs->freereg;
        luaK_reserveregs(fs, 1);
        expdesc v_temp = *v;
        luaK_dischargevars(fs, &v_temp);
        luaK_exp2reg(fs, &v_temp, v_old.u.info);
        init_exp(&e_one, VKINT, 0); e_one.u.ival = 1;
        luaK_posfix(fs, OPR_ADD, &v_temp, &e_one, line);
        luaK_storevar(fs, &v_var, &v_temp);
        *v = v_old;
        break;
      }
...
static BinOpr subexpr (LexState *ls, expdesc *v, int limit) {
  ...
  while (op != OPR_NOBINOPR && priority[op].left > limit) {
    ...
    if (op == OPR_COALESCE) {
      int end_label = NO_JUMP;
      luaK_dischargevars(ls->fs, v);
      if (v->k != VNONRELOC || v->u.info < ls->fs->nactvar) {
          luaK_exp2nextreg(ls->fs, v);
      }
      int reg = v->u.info;
      luaK_codeABC(ls->fs, OP_TESTNIL, reg, 0, 0);
      luaK_concat(ls->fs, &end_label, luaK_jump(ls->fs));
      nextop = subexpr(ls, &v2, priority[op].right);
      luaK_exp2reg(ls->fs, &v2, reg);
      luaK_patchtohere(ls->fs, end_label);
      v->k = VNONRELOC; v->u.info = reg;
      ls->fs->freereg = reg + 1;
      op = nextop;
    } else if (op == OPR_PIPE || op == OPR_BPIPE || op == OPR_SPIPE) {
      expdesc f_exp;
      int is_safe = (op == OPR_SPIPE);
      int end_label = NO_JUMP;
      if (is_safe) {
        luaK_dischargevars(ls->fs, v);
        luaK_exp2nextreg(ls->fs, v);
        luaK_codeABC(ls->fs, OP_TESTNIL, v->u.info, 0, 1);
        luaK_concat(ls->fs, &end_label, luaK_jump(ls->fs));
      }
      nextop = subexpr(ls, &f_exp, priority[op].right);
      if (op == OPR_PIPE || op == OPR_SPIPE) {
        luaK_exp2nextreg(ls->fs, &f_exp);
        int base = f_exp.u.info;
        luaK_exp2reg(ls->fs, v, base + 1);
        luaK_codeABC(ls->fs, OP_CALL, base, 2, 2);
        v->k = VNONRELOC; v->u.info = base;
        ls->fs->freereg = base + 1;
      } else {
        luaK_exp2nextreg(ls->fs, v);
        int base = v->u.info;
        luaK_exp2reg(ls->fs, &f_exp, base + 1);
        luaK_codeABC(ls->fs, OP_CALL, base, 2, 2);
        v->k = VNONRELOC; v->u.info = base;
        ls->fs->freereg = base + 1;
      }
      if (is_safe) luaK_patchtohere(ls->fs, end_label);
      op = nextop;
    } else {
...
static void classstat (LexState *ls, int line) {
  ...
  expdesc base_exp, class_var;
  if (testnext(ls, TK_EXTENDS)) {
    expr(ls, &base_exp);
  } else {
    init_exp(&base_exp, VNIL, 0);
  }

  /* 1. Create 'super' local variable and initialize it */
  new_localvar(ls, luaS_new(ls->L, "super"));
  luaK_exp2nextreg(fs, &base_exp);
  adjustlocalvars(ls, 1);
  int super_reg = fs->nactvar - 1;

  /* 2. Call name = newclass("name", super) */
  expdesc f;
  TString *newclass_s = luaS_new(ls->L, "newclass");
  singlevaraux(fs, newclass_s, &f, 1);
  if (f.k == VVOID) {
    expdesc key;
    singlevaraux(fs, ls->envn, &f, 1);
    codestring(ls, &key, newclass_s);
    luaK_indexed(fs, &f, &key);
  }
  luaK_exp2nextreg(fs, &f);
  int base_reg = f.u.info;
  ...
  expdesc super_val;
  init_exp(&super_val, VLOCAL, super_reg);
  luaK_exp2nextreg(fs, &super_val);

  luaK_codeABC(fs, OP_CALL, base_reg, 3, 2);

  /* 3. Assign new class to its name */
  singlevaraux(fs, classname, &class_var, 1);
  if (class_var.k == VVOID) {
    expdesc key;
    singlevaraux(fs, ls->envn, &class_var, 1);
    codestring(ls, &key, classname);
    luaK_indexed(fs, &class_var, &key);
  }
  expdesc class_res;
  init_exp(&class_res, VNONRELOC, base_reg);
  luaK_storevar(fs, &class_var, &class_res);

  /* Enter class body block */
  testnext(ls, TK_DO);
  testnext(ls, '{');

  while (!block_follow(ls, 1) && ls->t.token != '}') {
    int old_free = fs->freereg;
    int is_static = testnext(ls, TK_STATIC);
    if (ls->t.token == TK_PUBLIC || ls->t.token == TK_PRIVATE || ls->t.token == TK_PROTECTED) {
      luaX_next(ls);
    }
    ...
    if (ls->t.token == TK_NAME || ls->t.token == TK_FUNCTION) {
      ...
      if (is_constructor) mname = luaS_new(ls->L, "constructor");

      expdesc k, method, class_ref;
      codestring(ls, &k, mname);
      init_exp(&class_ref, VNONRELOC, base_reg);

      if (ls->t.token == '(') {
        body(ls, &method, !is_static, ls->linenumber);
        luaK_indexed(fs, &class_ref, &k);
        luaK_storevar(fs, &class_ref, &method);
      } else if (testnext(ls, '=')) {
        expr(ls, &method);
        luaK_indexed(fs, &class_ref, &k);
        luaK_storevar(fs, &class_ref, &method);
  ...
  if (testnext(ls, '}')) {}
  else check_match(ls, TK_END, TK_CLASS, line);
}
