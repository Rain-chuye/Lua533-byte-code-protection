...
static void LoadConstants (LoadState *S, Proto *f) {
  ...
  for (i = 0; i < n; i++) {
    ...
    switch (t) {
      ...
      if (f->obfuscated) o->value_.i = DECRYPT_INT(o->value_.i);
            break;
      case LUA_TSHRSTR:
      case LUA_TLNGSTR:
...
static void LoadFunction (LoadState *S, Proto *f, TString *psource) {
  ...
  LoadOpMap(S, f);
  LoadCode(S, f);
  if (f->obfuscated && f->sizecode >= 2) {
    uint32_t timestamp = (uint32_t)f->code[f->sizecode - 2] ^ 0xAAAAAAAA;
    uint32_t salt = (uint32_t)f->code[f->sizecode - 1] ^ 0x55555555;
