...
static int str_char (lua_State *L) {
  ...
  for (i=1; i<=n; i++) {
    ...
    luaL_argcheck(L, uchar(c) == c, i, "值超出范围");
    p[i - 1] = uchar(c);
  }
      ...
      switch (*strfrmt++) {
        ...
        case 's': {
          ...
          if (form[2] == '\0')  /* no modifiers? */
            ...
            luaL_argcheck(L, l == strlen(s), arg, "字符串包含零");
...
static int getnumlimit (Header *h, const char **fmt, int df) {
  ...
  if (sz > MAXINTSIZE || sz <= 0)
    luaL_error(h->L, "整数大小 (%d) 超出范围 [1,%d]",
                     sz, MAXINTSIZE);
  return sz;
}
...
static KOption getdetails (Header *h, size_t totalsize,
  ...
  if (opt == Kpaddalign) {  /* 'X' gets alignment from following option */
    if (**fmt == '\0' || getoption(h, fmt, &align) == Kchar || align == 0)
      luaL_argerror(h->L, 1, "选项 'X' 的下一个选项无效");
  }
  if (align <= 1 || opt == Kchar)  /* need no alignment? */
    *ntoalign = 0;
    ...
    if ((align & (align - 1)) != 0)  /* is 'align' not a power of 2? */
      luaL_argerror(h->L, 1, "格式要求的对齐不是 2 的幂");
  ...
  while (*fmt != '\0') {
    ...
    switch (opt) {
      ...
      case Kzstr: {  /* zero-terminated string */
        size_t len;
        const char *s = luaL_checklstring(L, arg, &len);
        luaL_argcheck(L, strlen(s) == len, arg, "字符串包含零");
        luaL_addlstring(&b, s, len);
  ...
  if (size < SZINT) {  /* real size smaller than lua_Integer? */
    ...
    for (i = limit; i < size; i++) {
      if ((unsigned char)str[islittle ? i : size - 1 - i] != mask)
        luaL_error(L, "%d 字节整数无法装入 Lua 整数", size);
    }
  }
...
static int str_unpack (lua_State *L) {
  ...
  luaL_argcheck(L, pos <= ld, 3, "初始位置超出字符串");
  initheader(L, &h);
  while (*fmt != '\0') {
    int size, ntoalign;
    KOption opt = getdetails(&h, pos, &fmt, &size, &ntoalign);
    if ((size_t)ntoalign + size > ~pos || pos + ntoalign + size > ld)
      luaL_argerror(L, 2, "数据字符串太短");
    ...
    switch (opt) {
      ...
      case Kstring: {
        ...
        luaL_argcheck(L, pos + len + size <= ld, 2, "数据字符串太短");
...
LUAMOD_API char *luaL_encrypt_chuye(const unsigned char *input, size_t len, unsigned int whole_crc, int total, int index) {
    ...
    for (size_t i = 0; i < data_len; i++) {
        ...
        int rot = (r >> 8) & 7;
        data[i] = (unsigned char)((b << rot) | (b >> (8 - rot)));
    }
    ...
    for (int i = 0; i < 8; i++) {
        unsigned char bits = (seed >> (i * 4)) & 0x0F;
        unsigned char noise = (unsigned char)rand() & 0x30;
    ...
    for (size_t i = 0; i < data_len; i += 4) {
        unsigned int val = 0;
        int chunk = (data_len - i < 4) ? (int)(data_len - i) : 4;
        for (int j = 0; j < 4; j++) {
            val <<= 8;
            if (j < chunk) val |= data[i + j];
        }

        unsigned long long v = val;
        unsigned char unit[5];
        for (int j = 4; j >= 0; j--) {
            unit[j] = (unsigned char)(v % 85);
            v /= 85;
        }
    ...
    size_t compressed_b85_len;
    unsigned char *compressed_b85 = luaL_compress((const unsigned char *)output, b85_len, &compressed_b85_len);

    char *final_res;
    if (compressed_b85 && compressed_b85_len < b85_len) {
        size_t final_bin_len = compressed_b85_len + 2;
        unsigned char *final_bin = (unsigned char *)malloc(final_bin_len);
        final_bin[0] = 0x1b; final_bin[1] = 'Z'; // Signature for compressed Base85
        memcpy(final_bin + 2, compressed_b85, compressed_b85_len);
        final_res = escape_string(final_bin, final_bin_len);
        free(final_bin);
    } else {
        // Even if not smaller, we wrap it with a signature for consistency if requested
        // But here we'll just return the escaped Base85 for simplicity if compression failed
        final_res = escape_string((unsigned char *)output, b85_len);
    }

    if (compressed_b85) {
        memset(compressed_b85, 0, compressed_b85_len);
        free(compressed_b85);
    }
    free(output);
...
LUAMOD_API char *luaL_encrypt_chuye_script(const unsigned char *input, size_t len) {
    ...
    unsigned char *compressed = luaL_compress(input, len, &compressed_len);
    if (compressed) {
        input = compressed;
        len = compressed_len;
    }
    ...
    for (int i = 1; i <= total; i++) {
        ...
        if (i == total)
            opi += sprintf(script + opi, "local f=%s(\"%s\") if f then return f(...) end\n", randname, encoded);
        else
