...
LUAMOD_API char *luaL_encrypt_chuye(const unsigned char *input, size_t len, unsigned int whole_crc, int total, int index) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        output[opi++] = B64_ALPHABET_MASTER[bits | noise];
    }
    ...
    for (size_t i = 0; i < data_len; i++) {
        ...
        while (bit_cnt >= 6) {
            bit_cnt -= 6;
            unsigned char val = (bit_buf >> bit_cnt) & 0x3F;
            bit_buf &= (1 << bit_cnt) - 1;
            unsigned int r_noise = xorshift128(&s_enc);
            if (r_noise % 13 == 0) {
                output[opi++] = alphabet[xorshift128(&s_enc) % 64];
            }
            unsigned int r_data = xorshift128(&s_enc);
            output[opi++] = alphabet[(val + (r_data % 64)) % 64];
        }
    }
    if (bit_cnt > 0) {
        unsigned char val = (bit_buf << (6 - bit_cnt)) & 0x3F;
        unsigned int r_noise = xorshift128(&s_enc);
        if (r_noise % 13 == 0) {
            output[opi++] = alphabet[xorshift128(&s_enc) % 64];
        }
