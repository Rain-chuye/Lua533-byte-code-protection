...
#define STRBUILDER_MT "String.Builder"
...
static int sb_append(lua_State *L) {
  StringBuilder *sb = (StringBuilder *)luaL_checkudata(L, 1, STRBUILDER_MT);
  size_t l;
  const char *s = luaL_checklstring(L, 2, &l);
  if (sb->len + l > sb->cap) {
    size_t newcap = (sb->len + l) * 2;
    sb->ptr = (char *)luaM_realloc_(L, sb->ptr, sb->cap, newcap);
    sb->cap = newcap;
  }
  memcpy(sb->ptr + sb->len, s, l);
  sb->len += l;
  lua_settop(L, 1);
  return 1;
}
...
static int sb_tostring(lua_State *L) {
  StringBuilder *sb = (StringBuilder *)luaL_checkudata(L, 1, STRBUILDER_MT);
  lua_pushlstring(L, sb->ptr, sb->len);
  return 1;
}
...
static int sb_len(lua_State *L) {
  StringBuilder *sb = (StringBuilder *)luaL_checkudata(L, 1, STRBUILDER_MT);
  lua_pushinteger(L, sb->len);
  return 1;
}

static int str_newbuilder(lua_State *L) {
  size_t init_cap = luaL_optinteger(L, 1, 128);
  StringBuilder *sb = (StringBuilder *)lua_newuserdata(L, sizeof(StringBuilder));
  sb->ptr = (char *)luaM_malloc(L, init_cap);
  sb->len = 0;
  sb->cap = init_cap;
  if (luaL_newmetatable(L, STRBUILDER_MT)) {
    lua_pushcfunction(L, sb_append);
    lua_setfield(L, -2, "append");
    lua_pushcfunction(L, sb_tostring);
    lua_setfield(L, -2, "tostring");
    lua_pushcfunction(L, sb_gc);
    lua_setfield(L, -2, "__gc");
    lua_pushcfunction(L, sb_tostring);
    lua_setfield(L, -2, "__tostring");
    lua_pushcfunction(L, sb_len);
