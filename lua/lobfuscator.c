...
static void fuse_instructions_internal(Proto *f) {
    ...
    for (int i = 0; i < f->sizecode - 1; i++) {
        ...
        /* Pattern: Ra = Rb[Rc]; if Ra <=> C then Ra = Ra else pc++ (TESTSET) */
        if (op1 == OP_GETTABLE && op2 == OP_TESTSET &&
            GETARG_A(inst1) == GETARG_B(inst2)) {
            int ra = GETARG_A(inst2);
            int rb = GETARG_B(inst1);
            int rc = GETARG_C(inst1);
            int test_c = GETARG_C(inst2);
            f->code[i] = CREATE_ABC(OP_FUSE_GETTABLE_TEST, ra, rb, rc);
            f->code[i+1] = CREATE_Ax(OP_EXTRAARG, test_c);
            i++; continue;
        }
        /* Pattern: Ra = Rb[Rc]; if (Ra == Rd) ~= A then pc++ (EQ) */
        if (op1 == OP_GETTABLE && op2 == OP_EQ &&
            GETARG_A(inst1) == GETARG_B(inst2)) {
            int ra = GETARG_A(inst1);
            int rb = GETARG_B(inst1);
            int rc = GETARG_C(inst1);
            int rd = GETARG_C(inst2);
            int eq_a = GETARG_A(inst2);
            f->code[i] = CREATE_ABC(OP_FUSE_GETTABLE_EQ, ra, rb, rc);
            f->code[i+1] = CREATE_Ax(OP_EXTRAARG, (eq_a << 9) | rd);
            i++; continue;
        }
...
static void obfuscate_proto_recursive(lua_State *L, Proto *f, int encrypt_k, uint32_t timestamp) {
    if (f->obfuscated) return;

    // Increase stack size for instruction fusion and dynamic decryption
    f->scratch_base = f->maxstacksize;
    if (f->maxstacksize <= 251) f->maxstacksize += 4;
    else { f->maxstacksize = 255; f->scratch_base = 251; }

    if (encrypt_k) {
        // We now encrypt integers only during Dump to improve VM performance
    }
    ...
    uint32_t metadata[2] = {timestamp, salt};
    f->inst_seed = murmurhash3_32(metadata, sizeof(metadata), 0x31337);

    /* Hide metadata at the end of the code array */
    int original_sizecode = f->sizecode;
    f->code = cast(Instruction *, luaM_reallocv(L, f->code, original_sizecode, original_sizecode + 2, sizeof(Instruction)));
    f->code[original_sizecode] = timestamp ^ 0xAAAAAAAA;
    f->code[original_sizecode + 1] = salt ^ 0x55555555;
    f->sizecode = original_sizecode + 2;

    /* Apply encryption after all optimizations to maintain patterns as long as possible */
    for (int i = 0; i < original_sizecode; i++) {
    ...
    for (int i = 0; i < f->sizep; i++) {
        obfuscate_proto_recursive(L, f->p[i], encrypt_k, timestamp);
    }
}
...
void obfuscate_proto(lua_State *L, Proto *f, int encrypt_k) {
    ...
    obfuscate_proto_recursive(L, f, encrypt_k, timestamp);
}
