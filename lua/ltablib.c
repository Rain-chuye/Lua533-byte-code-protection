...
static int tcreate (lua_State *L) {
  lua_Integer asize = luaL_checkinteger(L, 1);
  lua_Integer hsize = luaL_optinteger(L, 2, 0);
  lua_createtable(L, (int)asize, (int)hsize);
  return 1;
}
#endif
//---
...
static int tinsert (lua_State *L) {
  ...
  switch (lua_gettop(L)) {
    ...
    case 3: {
      ...
      if (lua_type(L, 1) == LUA_TTABLE && lua_getmetatable(L, 1) == 0) {
        Table *t = hvalue((const TValue *)lua_toluaobject(L, 1));
        if (e <= t->sizearray) {
          if (e > pos)
            memmove(&t->array[pos], &t->array[pos - 1], (size_t)(e - pos) * sizeof(TValue));
          setobj2t(L, &t->array[pos - 1], (const TValue *)lua_toluaobject(L, 3));
          luaC_barrierback(L, t, (const TValue *)lua_toluaobject(L, 3));
          return 0;
        }
      }
...
static int tremove (lua_State *L) {
  ...
  if (lua_type(L, 1) == LUA_TTABLE && lua_getmetatable(L, 1) == 0) {
    ...
    if (size <= t->sizearray) {
      if (pos < size)
        memmove(&t->array[pos - 1], &t->array[pos], (size_t)(size - pos) * sizeof(TValue));
...
static int tmove (lua_State *L) {
  ...
  if (e >= f) {  /* otherwise, nothing to move */
    ...
    if (lua_type(L, 1) == LUA_TTABLE && lua_getmetatable(L, 1) == 0 &&
        lua_type(L, tt) == LUA_TTABLE && lua_getmetatable(L, tt) == 0) {
      Table *src = hvalue((const TValue *)lua_toluaobject(L, 1));
      Table *dst = hvalue((const TValue *)lua_toluaobject(L, tt));
...
LUAMOD_API int luaopen_table (lua_State *L) {
  ...
  G(L)->tab_insert = tinsert;
  G(L)->tab_remove = tremove;
