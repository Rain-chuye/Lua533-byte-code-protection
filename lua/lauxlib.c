...
LUALIB_API unsigned char *luaL_decrypt_chuye(const char *input, size_t in_len, size_t *out_len) {
    ...
    unsigned char rev_alphabet[256];
    memset(rev_alphabet, 0xFF, 256);
    for (int i = 0; i < 64; i++) rev_alphabet[(unsigned char)alphabet[i]] = (unsigned char)i;
    ...
    while (in_pos < in_len) {
        ...
        int val = rev_alphabet[(unsigned char)input[in_pos++]];
