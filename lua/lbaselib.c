...
static int luaB_vmprotect (lua_State *L) {
...
static int chunk_handler(lua_State *L, unsigned char *decoded, size_t dlen, int envidx, const char *mode) {
    ...
    memcpy(expected_hash, decoded + 12, 32);
    unsigned char actual_hash[32];
    luaL_sha256(decoded + 12 + 32, dlen - 12 - 32, actual_hash);
    if (memcmp(expected_hash, actual_hash, 32) != 0) {
        memset(decoded, 0, dlen);
        free(decoded);
        return luaL_error(L, "字节码完整性校验失败 (SHA256)");
    }
    ...
    if (lua_rawgeti(L, 6, (lua_Integer)index) == LUA_TNIL) {
        lua_pop(L, 1);
        lua_pushlstring(L, (const char *)decoded + 12 + 32, dlen - 12 - 32);
...
static int luaB_load (lua_State *L) {
  ...
  if (s != NULL && l > 0) {
    ...
    if (decoded && dlen >= (12 + 32) && memcmp(decoded, "CHYE", 4) == 0) {
...
static int luaB_newinstance (lua_State *L) {
  int n = lua_gettop(L);
  if (lua_type(L, 1) != LUA_TTABLE) {
      return luaL_error(L, "newinstance 期望 table，得到 %s", luaL_typename(L, 1));
  }
  lua_newtable(L); // index n+1
  lua_insert(L, 2); // move it to index 2
  lua_pushvalue(L, 1);
  lua_setmetatable(L, 2);
  if (lua_getfield(L, 1, "constructor") == LUA_TFUNCTION) {
    lua_pushvalue(L, 2); // instance (self)
    for (int i = 3; i <= n + 1; i++) {
        lua_pushvalue(L, i);
    }
    lua_call(L, n, 0);
...
static int luaB_newclass (lua_State *L) {
  const char *name = luaL_checkstring(L, 1);
  lua_settop(L, 2);
  lua_newtable(L); // index 3: CLASS
  ...
  if (!lua_isnil(L, 2)) {
    ...
    lua_newtable(L); // index 4: METATABLE
    lua_pushvalue(L, 2);
    lua_setfield(L, 4, "__index");
    lua_setmetatable(L, 3);
  }
  lua_pushcfunction(L, luaB_newinstance);
