...
static const char CHUYE_CHUNKS_KEY = 'c';
...
static int chunk_handler(lua_State *L, unsigned char *decoded, size_t dlen, int envidx, const char *mode) {
    ...
    if (lua_rawgetp(L, LUA_REGISTRYINDEX, &CHUYE_CHUNKS_KEY) == LUA_TNIL) { // Index 5
        lua_pop(L, 1);
        lua_newtable(L);
        lua_pushvalue(L, -1);
        lua_rawsetp(L, LUA_REGISTRYINDEX, &CHUYE_CHUNKS_KEY);
    }
    ...
    if (lua_rawgeti(L, 6, (lua_Integer)index) == LUA_TNIL) {
        ...
        if (count == expected) {
            ...
            // Clean up the main table if empty? No, keep it.
