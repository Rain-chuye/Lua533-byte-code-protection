...
void luaX_setinput (lua_State *L, LexState *ls, ZIO *z, TString *source,
  ...
  ls->interp_delim = 0;
  ls->interp_step = 0;
  ls->interp_brace_level = 0;
  ls->interp_temp_ts = NULL;
...
static void read_string (LexState *ls, int del, SemInfo *seminfo, int is_part, int *hit_interp) {
  *hit_interp = 0;
  if (!is_part) save_and_next(ls);  /* keep delimiter (for error messages) */
  ...
  if (!is_part) save_and_next(ls);  /* skip delimiter */
  else next(ls);
end_string:
  if (!is_part)
    seminfo->ts = luaX_newstring(ls, luaZ_buffer(ls->buff) + 1,
                                     luaZ_bufflen(ls->buff) - (*hit_interp ? 1 : 2));
  else
...
static int llex (LexState *ls, SemInfo *seminfo) {
  ...
  if (ls->interp_step > 0) {
    ...
    switch (step) {
      ...
      case 2:
        ls->interp_step = 3;
        seminfo->ts = luaS_new(ls->L, "tostring");
        return TK_NAME;
      case 3:
        ls->interp_step = 0;
        ls->interp_brace_level = 0;
      ...
      case 7: {
        int hit;
        read_string(ls, ls->interp_delim, seminfo, 1, &hit);
        if (hit) {
          ls->interp_step = 1;
          return TK_STRING;
        } else {
  ...
  for (;;) {
    switch (ls->current) {
      ...
      case '?': {
        ...
        if (check_next1(ls, '.')) return TK_OPTIONAL;
        return '?';
      }
      case '<': {
        next(ls);
        ...
        else if (check_next1(ls, '|')) return TK_BPIPE;
...
//mod by nirenr
      ...
      case '|': {
         ...
         if (check_next1(ls, '?')) {
           if (check_next1(ls, '>')) return TK_SPIPE;
         }
         return '|';
      }
//---
      ...
      case '+': {
        ...
        if (check_next1(ls, '+')) return TK_PPLUS;
        else return '+';
      }
      case '*': {
        next(ls);
      ...
      case '"': case '\'': {  /* short literal strings */
        int hit;
        int del = ls->current;
        read_string(ls, del, seminfo, 0, &hit);
        if (hit) {
          ls->interp_delim = del;
          ls->interp_temp_ts = seminfo->ts;
          ls->interp_step = 11;
          return '(';
        }
        return TK_STRING;
      ...
      case '{': {
        if (ls->interp_delim != 0) ls->interp_brace_level++;
        next(ls);
        return '{';
      }
      case '}': {
        if (ls->interp_delim != 0 && ls->interp_brace_level == 0) {
          next(ls);
          ls->interp_step = 5;
      ...
      default: {
        if (lislalpha(ls->current)) {  /* identifier or reserved word? */
          ...
          ts = luaX_newstring(ls, luaZ_buffer(ls->buff),
                                  luaZ_bufflen(ls->buff));
          if (strcmp(getstr(ts), "_raw") == 0) {
            if (ls->current == '"' || ls->current == '\'') {
              int del = ls->current;
              luaZ_resetbuffer(ls->buff);
              next(ls);
              while (ls->current != del && ls->current != EOZ) {
                save_and_next(ls);
              }
              if (ls->current == del) next(ls);
