...
LUAMOD_API char *luaL_encrypt_chuye(const unsigned char *input, size_t len, unsigned int whole_crc, int total, int index) {
    ...
    size_t out_capacity = (data_len * 3) + 256;
    char *output = (char *)malloc(out_capacity);
    size_t opi = 0;

    for (int i = 0; i < 8; i++) {
        unsigned char bits = (unsigned char)((seed >> (i * 4)) & 0x0F);
    ...
    for (size_t i = 0; i < data_len; i++) {
        ...
        while (bit_cnt >= 6) {
            bit_cnt -= 6;
            unsigned char val = (unsigned char)((bit_buf >> bit_cnt) & 0x3F);
            bit_buf &= (1 << bit_cnt) - 1;
            unsigned int r_noise = xorshift128(&s_enc);
            if (r_noise % 13 == 0) output[opi++] = alphabet[xorshift128(&s_enc) % 85];
            unsigned int r_data = xorshift128(&s_enc);
            output[opi++] = alphabet[(val + (r_data % 85)) % 85];
        }
    }
    if (bit_cnt > 0) {
        unsigned char val = (unsigned char)((bit_buf << (6 - bit_cnt)) & 0x3F);
        unsigned int r_noise = xorshift128(&s_enc);
        if (r_noise % 13 == 0) output[opi++] = alphabet[xorshift128(&s_enc) % 85];
        unsigned int r_data = xorshift128(&s_enc);
        output[opi++] = alphabet[(val + (r_data % 85)) % 85];
