...
#define ChyeDecrypt(inst, idx) { uint32_t _k = (cl->p->inst_seed) ^ ((uint32_t)(idx) * 0x9E3779B9U); if (((_k | ~_k) & 0xFFFFFFFF) == 0xFFFFFFFF) { inst -= _k; inst = (inst >> 13) | (inst << 19); inst ^= _k; } }
...
#define donextjump(ci)	{ \
    i = *ci->u.l.savedpc; \
    ChyeDecrypt(i, (int)(ci->u.l.savedpc - cl->p->code)); \
    dojump(ci, i, 1); \
}


#define SaveVPC() { ci->u.l.vpc = vpc; ci->u.l.vcount = vcount; ci->u.l.vpc_idx = vpc_idx; }
#define Protect(x) { SaveVPC(); {x;}; base = ci->u.l.base; ra = RA(i); vpc = ci->u.l.vpc; vcount = ci->u.l.vcount; vpc_idx = ci->u.l.vpc_idx; }
...
#define vmfetch()	{ \
  ...
  i = *(ci->u.l.savedpc++); \
  ChyeDecrypt(i, (int)(ci->u.l.savedpc - cl->p->code - 1)); \
...
void luaV_execute (lua_State *L) {
  ...
  vpc = ci->u.l.vpc; vcount = ci->u.l.vcount; vpc_idx = ci->u.l.vpc_idx;
  ...
  for (;;) {
    L_next_ins: {
      ...
      vmcase(OP_LOADKX) {
        ...
        ChyeDecrypt(next_i, (int)(ci->u.l.savedpc - cl->p->code));
      ...
      vmcase(OP_FUSE_PARTICLE_DIST) {
        ...
        ChyeDecrypt(next_i1, (int)(ci->u.l.savedpc - cl->p->code - 1));
        Instruction next_i2 = *ci->u.l.savedpc++;
        ChyeDecrypt(next_i2, (int)(ci->u.l.savedpc - cl->p->code - 1));
      ...
      vmcase(OP_VIRTUAL) {
        ...
        if (cl->p->vcode && vindex >= 0 && vindex < cl->p->sizevcode) {
          ...
          ChyeDecrypt(vcount_raw, vpc_idx++); vcount = (int)vcount_raw;
